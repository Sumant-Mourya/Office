...
def graphs_page():
    ...
    with ui.header().classes("app-header items-center justify-between px-6 py-3"):
        with ui.row().classes("items-center gap-3"):
            ui.html(CONSOLE_HEADER_ICON)
            ui.label("Analytics Dashboard").classes("text-lg font-bold text-white tracking-wide")
        with ui.row().classes("gap-3"):
            ui.button("Dashboard", on_click=lambda: ui.navigate.to("/dashboard")).props("flat color=white").classes("text-sm font-semibold")
            ui.button("Logout", on_click=_logout).props("flat color=red").classes("text-sm font-semibold")
            ...
            with data_container:
                for day_data in all_days:
                    ...
                    with ui.expansion(
                        ...
                        total_work = sum(h.get("work_seconds", 0) for h in hours.values())
                        total_idle = sum(h.get("idle_seconds", 0) for h in hours.values())
                        total_mouse = sum(h.get("mouse_clicks", 0) for h in hours.values())
                        total_keys = sum(h.get("key_presses", 0) for h in hours.values())

                        with ui.row().classes("w-full items-center justify-between p-4 bg-gray-900/40 border-b border-gray-800"):
                            with ui.row().classes("gap-4"):
                                ui.markdown(f"**Work Time:** `{_fmt(total_work)}`").classes("text-sm text-emerald-400")
                                ui.markdown(f"**Idle Time:** `{_fmt(total_idle)}`").classes("text-sm text-rose-400")
                            with ui.row().classes("gap-4"):
                        ...
                        if not hours:
                            ui.label("No metrics gathered for this day.").classes("text-gray-600 p-4")
                            continue
                        ...
                        for slot in sorted_slots:
                            ...
                            work_vals.append(round(h.get("work_seconds", 0) / 60, 1))
                            idle_vals.append(round(h.get("idle_seconds", 0) / 60, 1))
                        ...
                        with ui.row().classes("w-full gap-4 p-4"):
                            ...
                            with ui.card().classes("chart-card flex-1"):
                                ...
                                ui.echart({
                                    ...
                                    "xAxis": {"type": "category", "data": chart_labels, "axisLabel": {"color": "#9ca3af"}},
                                    "yAxis": {"type": "value", "axisLabel": {"color": "#9ca3af"}},
                                    "series": [
                                        {"name": "Mouse Clicks", "type": "bar", "data": mouse_vals, "itemStyle": {"color": "#6366f1"}},
                                        {"name": "Key Presses", "type": "bar", "data": key_vals, "itemStyle": {"color": "#10b981"}},
                                    ],
                                }).style("height: 280px")
                            ...
                            with ui.card().classes("chart-card flex-1"):
                                ...
                                ui.echart({
                                    "backgroundColor": "transparent",
                                    "tooltip": {"trigger": "axis"},
                                    "legend": {"data": ["Work (min)", "Idle (min)"], "textStyle": {"color": "#9ca3af"}, "bottom": 0},
                                    "grid": {"left": "3%", "right": "3%", "bottom": "15%", "top": "5%", "containLabel": True},
                                    "xAxis": {"type": "category", "data": chart_labels, "axisLabel": {"color": "#9ca3af"}},
                                    "yAxis": {"type": "value", "axisLabel": {"color": "#9ca3af"}},
                                    "series": [
                                    ...
                                    with ui.row().classes("w-full gap-8 p-3"):
                                        ...
                                        if sites:
                                            with ui.column().classes("flex-1"):
                                                ...
                                                for s, sec in sorted(sites.items(), key=lambda x: -x[1])[:8]:
